...
// Реалізація функції введення рядка
const int32_t get_str(const char const* msg, char* str, const int32_t limit)
{
    ...
    if (fgets(str, limit, stdin)) {
        // Ручне видалення символу нового рядка '\n', якщо він є
        int i = 0;
        while (str[i] != '\0') {
            if (str[i] == '\n') {
                str[i] = '\0';
...
// Реалізація функції довжини рядка
...
// Реалізація функції копіювання рядка з лімітом
void strcopy(char* fStr, char* sStr, size_t until)
{
    ...
    fStr[i] = '\0'; // Гарантуємо нуль-термінатор
}

// Реалізація функції порівняння рядків
...
char* process_string(char* str)
{
    ...
    for (size_t i = 0; i < length; i++)
    {
        ...
        if (str[i + 1] != ' ')
        {
            free(result); // Важливо очистити пам'ять перед поверненням NULL
            return NULL;
        }
        strcatt(result, ": ");
        ...
        for (size_t j = i + 2ull, strRealCnt = 0ull; j < length; j++)
        {
            ...
            {
                // Перевірка на переповнення масиву (необов'язково, але бажано)
